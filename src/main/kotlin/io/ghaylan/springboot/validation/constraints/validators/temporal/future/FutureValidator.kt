...
object FutureValidator: ConstraintValidator<Temporal, FutureConstraint>() {
	...
	override suspend fun validate(
		...
		if (withinErrors.isNotEmpty() && value.isAfter(maxTemporal)) {
			return ApiError(code = ApiErrorCode.FUTURE_VIOLATION,
				message = "Must be in the future, within the next ${withinErrors.joinToString(", ")}.")
		}
	...
	override fun applicableErrorCodes(): Array<ApiErrorCode> = arrayOf(ApiErrorCode.FUTURE_VIOLATION)
}