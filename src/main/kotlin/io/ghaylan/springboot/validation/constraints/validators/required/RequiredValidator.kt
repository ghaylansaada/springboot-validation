...
object RequiredValidator: ConstraintValidator<Any, RequiredConstraint>() {
	...
	override suspend fun validate(
		...
		if (constraint.condition == RequirementCondition.IF_DEPENDENT_NULL && dependentField == null) {
			return ApiError(code = ApiErrorCode.REQUIRED_VIOLATION,
				message = "Required because the related field '${constraint.dependentField}' is missing")
		}
		
		if (constraint.condition == RequirementCondition.IF_DEPENDENT_NOT_NULL && dependentField != null) {
			return ApiError(code = ApiErrorCode.REQUIRED_VIOLATION,
				message = "Required because the related field '${constraint.dependentField}' is provided")
		}
	...
	override fun applicableErrorCodes(): Array<ApiErrorCode> = arrayOf(ApiErrorCode.REQUIRED_VIOLATION)
}